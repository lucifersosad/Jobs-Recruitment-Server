...
export async function areaDetails(input: string): Promise<any> {
  try {
    const { data } = await axios.post(
      process.env.DIVISION_AUTOCOMPLETE,
      {
        components,
        input: [{ text: input }],
        use_case,
    ...
    console.log("🚀 ~ areaDetails ~ error:", error);
    return [];
  }
}
export async function detailedAddress(
  ...
  try {
    ...
    console.log("🚀 ~ detailedAddress ~ error:", error);
    return [];
  }
}

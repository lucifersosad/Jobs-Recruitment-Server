...
const JobCategory = require("./models/jobCategories.model.ts");
...
const jobCategoriesSchema = new mongoose.Schema(
  {
      ...
      thumbnail:{type: String, default: "https://res.cloudinary.com/dmmz10szo/image/upload/v1703728915/vgmvvcn0te8lhdrbued1.webp"},
      description:String,
      deletedAt:Date,
      deleted: {
          type: Boolean,
  ...
  try {
    ...
    const parentDocs = await JobCategory.insertMany(
      data.map(item => ({
        ...
        slug: slug(item.attributes?.nameVi.replaceAll("/", " "), { lower: true }),
        keyword: slug(item.attributes?.nameVi.replaceAll("/", " "), { lower: true })
      }))
    );
